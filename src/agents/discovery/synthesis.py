...
def synthesize(
    ...
    if llm_clusters:
        ...
        prompt = _build_synthesis_prompt(
            llm_clusters,
            [{"title": a["title"], "slug": a["slug"]} for a in published_articles],
            rejected_topics,
        )
        try:
            llm_results = _call_synthesis_llm(prompt, anthropic_client)
        except Exception as e:
            log.error("[discovery] synthesis LLM call failed: %s", e)
            llm_results = [
                {"index": j, "drop": True, "drop_reason": f"synthesis call failed: {e}"}
                for j in range(len(llm_clusters))
            ]
        ...
        for r in llm_results:
            ...
            source_count_score = min(1.0, len(set(item["source_id"] for item in relevant_items)) / 3)
