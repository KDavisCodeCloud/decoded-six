...
const KNOWN_AGENTS = [
  { name: 'DS-DRAFT',      desc: 'Draft generation',           schedule: 'Weekly' },
  ...
  { name: 'DS-PROD',       desc: 'Product scout',              schedule: 'Gate B+' },
  { name: 'DS-AFF',        desc: 'Affiliate scout',            schedule: 'Gate 2+' },
]
...
export default async function AgentsPage() {
  ...
  recentLogs.forEach(log => {
    if (!lastRun.has(log.agent_name)) lastRun.set(log.agent_name, log)
  })
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-quiet text-sm mt-1">11 agents registered — build phases 3–7</p>
      </div>
          ...
          return (
            <div key={agent.name} className="dash-card p-4 flex items-center gap-4">
              <div className={`w-2.5 h-2.5 rounded-full shrink-0 ${
                !last           ? 'bg-dash-border' :
                last.status === 'success' ? 'bg-green-400' :
                last.status === 'error'   ? 'bg-neon-pink' :
                last.status === 'running' ? 'bg-gta-gold animate-pulse' :
                'bg-quiet'
              }`} />
            ...
            {recentLogs.slice(0, 50).map(log => (
              <div key={log.id} className="px-6 py-3 flex items-center gap-4">
                <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                  ...
                  log.status === 'error'   ? 'bg-neon-pink' :
                  log.status === 'running' ? 'bg-gta-gold animate-pulse' :
                  'bg-quiet'
                }`} />
                <span className="text-gta-gold text-xs font-mono w-32 shrink-0">{log.agent_name}</span>
